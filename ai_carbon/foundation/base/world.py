...
class Maps:
    ...
    def set_point(self, entity_name, r, c, val, owner=None):
        ...
        if entity_name in self._private_landmark_types:
            ...
            assert o[r, c] == -1 or o[r, c] == int(owner), (o[r, c], int(owner), self.is_accessible(r, c, owner))
    ...
    def location_landmarks(self, r, c):
        ...
        landmarks = self._blocked + self._private
        """Return {landmark: health} dictionary for any landmarks at location [r, c]."""
        return {k: v for k, v in tmp.items() if k in landmarks and v > 0}

    def location_public(self, r, c):
        """Return {landmark: health} dictionary for any landmarks at location [r, c]."""
        return {k: self._maps[k][r, c] for k in self._public if self._maps[k][r, c] > 0}
...
class World:
    ...
    def loc_map(self):
        ...
        Locations with a value of -1.0.0 are not occupied by an agent.
        """
    ...
    def is_location_accessible(self, r, c, agent):
        ...
        return self.is_valid(r, c) and self.maps.is_accessible(r, c, agent.idx)

    def can_agent_occupy(self, r, c, agent):
        """Return True if location [r, c] is accessible to agent and unoccupied."""
        return self.is_location_accessible(r, c, agent) and self.maps.unoccupied[r, c]
    ...
    def set_agent_loc(self, agent, r, c):
        ...
        return agent.loc[0], agent.loc[1]
    ...
    def location_public(self, r, c):
        ...
        if not self.is_valid(r, c):
            return {}
        return self.maps.location_public(r, c)
    ...
    def location_blank(self, r, c):
        """Return {landmark: health} dictionary for any landmarks at location [r, c]."""
        if not self.is_valid(r, c):
            return False
        return self.maps.empty[r, c]

    def get_near_blank_grid(self, r, c):
        near_grids = [(r+1, c+1), (r, c+1), (r-1, c+1), (r+1, c), (r-1, c), (r+1, c), (r-1, c+1), (r-1, c), (r-1, c-1)]
        near_blank_grids = []
        for grid in near_grids:
            if self.location_blank(grid[0], grid[1]):
                near_blank_grids.append(grid)
        return near_blank_grids
